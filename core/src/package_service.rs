...
struct RetainedPackageServiceStorage(UnsafeCell<PackageService<'static>>);

// SAFETY:
// 1. Invariant: retained package service storage always contains an initialized
//    PackageService; the initialized flag controls syscall visibility.
// 2. Established by: the static initializer writes an empty PackageService, and
//    retained-service initializers flip the visibility flag with compare_exchange.
// 3. Lifetime: the retained service lives in static storage for the boot.
...
// 5. Alignment: UnsafeCell<PackageService> provides PackageService alignment.
...
static RETAINED_PACKAGE_SERVICE: RetainedPackageServiceStorage =
    RetainedPackageServiceStorage(UnsafeCell::new(PackageService::new_empty()));
...
pub(crate) fn initialize_retained_package_service_for_phase13(
    ...
    // SAFETY:
    // 1. Invariant: this is the only visibility transition and replacement
    //    write to the retained package service slot.
    ...
    //    exit or test-only reset replaces it with an empty service.
    // 4. Pointer ownership: RETAINED_PACKAGE_SERVICE owns the storage; the
    //    input service is moved and not used again by the caller.
    // 5. Alignment: UnsafeCell<PackageService> preserves PackageService alignment.
    ...
    unsafe {
        *RETAINED_PACKAGE_SERVICE.0.get() = service;
    }
    true
}

#[cfg(all(not(test), not(feature = "verify"), not(feature = "hardware-probe")))]
fn initialize_empty_retained_package_service_for_phase13() -> bool {
    RETAINED_PACKAGE_SERVICE_INITIALIZED
        .compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst)
        .is_ok()
}
...
pub(crate) fn with_retained_package_service_for_phase13<R>(
    ...
    // SAFETY:
    // 1. Invariant: the retained service slot always contains one initialized
    //    PackageService, and the flag gates whether callers may borrow it.
    // 2. Established by: static initialization creates an empty service, and
    //    retained initializers set the flag before callers can issue syscalls.
    ...
    Some(f(unsafe { &mut *RETAINED_PACKAGE_SERVICE.0.get() }))
}
...
pub(crate) fn initialize_package_service_from_device(
    ...
    if !initialize_empty_retained_package_service_for_phase13() {
        return Err(PackageStatus::BadRequest);
    }
    restore_retained_package_service_from_device(device)
}

#[cfg(all(not(test), not(feature = "verify"), not(feature = "hardware-probe")))]
fn restore_retained_package_service_from_device(
    device: BlockDeviceInfo,
) -> Result<(), PackageStatus> {
...
fn reset_retained_package_service_for_phase13_test() {
    if RETAINED_PACKAGE_SERVICE_INITIALIZED.swap(false, Ordering::SeqCst) {
        // SAFETY:
        // 1. Invariant: the retained service slot always contains one
        //    initialized PackageService.
        // 2. Established by: initialize_retained_package_service_for_phase13 is
        //    the only test writer and sets the flag before tests can use the slot.
        // 3. Lifetime: this test-only reset replaces the stored service with an
        //    empty retained service.
        // 4. Pointer ownership: tests hold the retained-service test lock and
        //    no service borrow while resetting.
        // 5. Alignment: UnsafeCell<PackageService> preserves alignment.
        // 6. Mapped length: exactly one PackageService is replaced.
        ...
        //    mutable reference.
        unsafe {
            *RETAINED_PACKAGE_SERVICE.0.get() = PackageService::new_empty();
        }
    }
}
...
impl PackageLaunchGrant {
    pub const fn from_handle(requirement_id: u16, capability: CapabilityHandle) -> Self {
        Self {
            requirement_id,
            capability: PackedCapability::from_parts(capability.slot(), capability.generation()),
        }
    }

    #[cfg_attr(not(test), allow(dead_code))]
    pub const fn from_packed(requirement_id: u16, capability: PackedCapability) -> Self {
        Self {
            requirement_id,
            capability,
        }
    ...
    const fn capability_handle(self) -> CapabilityHandle {
        CapabilityHandle::from_parts(self.capability.slot(), self.capability.generation())
    }
...
pub trait PackageLaunchGrantValidator {
    fn validate_launch_requirement_grant(
        &self,
...
impl PackageLaunchGrantValidator for CapabilityTable {
    fn validate_launch_requirement_grant(
        &self,
        caller: ActiveUserProcess,
        requirement: PackageLaunchRequirement,
...
impl PackageLaunchGrantValidator for ObjectService {
    fn validate_launch_requirement_grant(
        &self,
        caller: ActiveUserProcess,
        requirement: PackageLaunchRequirement,
        ...
        self.validate_package_launch_grant(
            caller,
            supplied.packed_capability(),
            requirement.resource,
            requirement.rights,
        )
        .map_err(|_| PackageStatus::FinalCapabilityDenied)
...
impl<'a> PackageService<'a> {
    ...
    pub fn launch(
        ...
        self.launch_with_validator(request, capabilities)
    }

    pub fn launch_with_validator<V: PackageLaunchGrantValidator + ?Sized>(
        &mut self,
        request: PackageLaunchRequest<'_>,
        validator: &V,
    ) -> Result<PackageLaunchResult, PackageStatus> {
...
mod tests {
    ...
    use crate::{
        ...
        object_relationships::{
            PACKAGE_LOCATOR_ROOT_OBJECT_ID, RelationshipKind, SHELL_WORKSPACE_OBJECT_ID,
        },
        object_service::{ObjectService, ObjectServiceError, PackageDefinedCreateInput},
        object_service_checkpoint::{
    ...
    use pythos_shared::{
        ...
        package_abi::{
            ...
            PACKAGE_CONTENT_MAX_STAGED_RECORDS, PACKAGE_DEFINED_STATE_FORMAT_EMPTY,
    ...
    fn package_disable_blocks_new_launch_without_tearing_down_running_context_or_grants() {
        ...
        let second_capability = capabilities
            ...
            .unwrap();
        let second_grants = [PackageLaunchGrant::from_handle(
            requirement.requirement_id,
            second_capability,
        )];
    ...
    fn package_launch_capability_final_authority_denial_returns_final_capability_denied() {
        ...
        package_service
            ...
            .unwrap();
        let supplied_grants = [PackageLaunchGrant::from_handle(7, wrong_holder_capability)];
    ...
    fn package_launch_capability_valid_explicit_grants_produce_request_with_only_supplied_grants() {
        ...
        package_service
            ...
            .unwrap();
        let supplied_grants = [PackageLaunchGrant::from_handle(7, supplied_capability)];
    ...
    #[test]
    fn package_launch_validates_object_service_owned_grant_and_preserves_packed_handle() {
        let mut package_service = service_with_launch_export();
        let mut object_service = ObjectService::new_for_test();
        let caller = ActiveUserProcess::new(CALLER_SERVICE, 0x504B_4C41_554E_5A01, 0x13);
        let workspace_capability = object_service.grant_workspace_capability(caller).unwrap();
        let requirement = PackageLaunchRequirement {
            requirement_id: 7,
            graph_import_slot: 0,
            resource: ResourceId::new(SHELL_WORKSPACE_OBJECT_ID),
        ...
        assert_eq!(launch.grant(0), Some(supplied_grants[0]));
        assert_eq!(graph_grant.capability, workspace_capability);

        let schema = object_service
            .create_schema_definition_object(
                caller,
                ObjectId::new(0x5059_5343_484F_5A01),
                ObjectId::new(0x5059_504B_474F_5A01),
                sha256(b"schema:object-service-launch"),
            )
            .unwrap();
        let created = object_service
            .create_package_defined_object(
    ...
    #[test]
    fn package_launch_rejects_same_numeric_handle_from_unrelated_table() {
        let mut package_service = service_with_launch_export();
        let object_service = ObjectService::new_for_test();
        let caller = ActiveUserProcess::new(CALLER_SERVICE, 0x504B_4C41_554E_5A02, 0x13);
        let mut unrelated_table = CapabilityTable::new();
        let unrelated_handle = unrelated_table
            .grant(
                caller.service_id(),
                ResourceId::new(SHELL_WORKSPACE_OBJECT_ID),
                RightsMask::new(RightsMask::WRITE),
            ...
            .unwrap();
        let unrelated_packed =
            PackedCapability::from_parts(unrelated_handle.slot(), unrelated_handle.generation());
        assert_eq!(
            unrelated_packed.raw(),
            object_service.shell_workspace_capability().raw()
        );
        let requirement = PackageLaunchRequirement {
            requirement_id: 7,
            graph_import_slot: 0,
        ...
        assert_eq!(
            package_service.launch_with_validator(
                PackageLaunchRequest {
                    caller,
                    namespace_root: ObjectId::new(PACKAGE_LOCATOR_ROOT_OBJECT_ID),
                    locator: "seed/launch",
    ...
    #[test]
    fn package_launch_rejects_stale_object_service_capability() {
        let mut package_service = service_with_launch_export();
        let mut object_service = ObjectService::new_for_test();
        let caller = ActiveUserProcess::new(CALLER_SERVICE, 0x504B_4C41_554E_5A03, 0x13);
        let workspace_capability = object_service.grant_workspace_capability(caller).unwrap();
        object_service
            .revoke_package_launch_capability_for_test(workspace_capability)
            .unwrap();
        let requirement = PackageLaunchRequirement {
            requirement_id: 7,
            graph_import_slot: 0,
    ...
    #[test]
    fn package_launch_rejects_object_service_capability_for_wrong_operation() {
        let mut package_service = service_with_launch_export();
        let mut object_service = ObjectService::new_for_test();
        let caller = ActiveUserProcess::new(CALLER_SERVICE, 0x504B_4C41_554E_5A04, 0x13);
        let workspace_capability = object_service.grant_workspace_capability(caller).unwrap();
        let requirement = PackageLaunchRequirement {
            requirement_id: 7,
            graph_import_slot: 0,
            resource: ResourceId::new(SHELL_WORKSPACE_OBJECT_ID),
            rights: RightsMask::new(RightsMask::SEND),
        };
        package_service
            .record_launch_requirement(
                ObjectId::new(PACKAGE_LOCATOR_ROOT_OBJECT_ID),
    ...
    #[test]
    fn package_launch_rejects_object_service_capability_for_wrong_target() {
        let mut package_service = service_with_launch_export();
        let mut object_service = ObjectService::new_for_test();
        let caller = ActiveUserProcess::new(CALLER_SERVICE, 0x504B_4C41_554E_5A05, 0x13);
        let workspace_capability = object_service.grant_workspace_capability(caller).unwrap();
        let requirement = PackageLaunchRequirement {
            requirement_id: 7,
            graph_import_slot: 0,
            resource: ResourceId::new(SHELL_WORKSPACE_OBJECT_ID ^ 1),
            rights: RightsMask::new(RightsMask::WRITE),
        };
        package_service
            .record_launch_requirement(
