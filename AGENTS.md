...
Milestone 1.5 and Phase 2 are complete. Phase 3 is active. The next locked slice is `service-identity`; before implementation, record the required Phase 3 ADRs for TCB invariants, capability token representation, and revocation semantics. Do not begin IPC channels, bounded queues, request/reply, shared memory, Python runtime, desktop, audio, storage, networking, AI, or hardware-expansion work.
