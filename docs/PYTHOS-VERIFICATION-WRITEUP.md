...
takes ownership of memory and execution from firmware, builds a native kernel
substrate, brings up service identity and capability mechanisms, persists typed
objects across reboot, and finishes with a bounded hardware-enforced ring-3
...
The project is currently through Phase 8, `real-hardware-isolation`. Later
phases such as networking, package management, updates, physical hardware
expansion, SMP, semantic indexing, and optional AI remain intentionally
unimplemented.
