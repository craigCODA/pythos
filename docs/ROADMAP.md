...
cutover/cross-target branch. Phase 7 changes normal boot to launch the
Pyth-native session-manager and default service composition, while retaining
the Rust object shell under the `legacy-shell` maintenance fallback and as the
recovery shell after a contained default-service fault.
