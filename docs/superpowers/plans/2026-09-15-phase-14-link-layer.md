...
- [ ] Build the loader, core with `--no-default-features --features link-layer-probe`, isolated native ELF, verified shell ELF, and image with `--link-layer-probe-elf`; assert all expected artifacts exist before starting QEMU.
...
- [ ] Run QEMU with `--no-audio-device --no-virtio-blk --virtio-net --virtio-net-peer-port <peer> --shell-port <shell> --expect-outcome success`; observe COM2 consumer markers, COM1 teardown/readiness markers, and runner success through one `AcceptanceTimeline`.
