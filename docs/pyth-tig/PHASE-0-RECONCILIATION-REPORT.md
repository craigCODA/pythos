...
- Branch: `docs/pythtig-phase0-from-physical-evidence`
- Base branch: `agent/physical-evidence-terminal`
- Base HEAD: `809f8bd docs: point evidence terminal record at hardened commit`
...
- The physical-evidence baseline is `agent/physical-evidence-terminal`, not
  `main`.
- On this baseline, ADR 0063's implementation files, Cargo features, and
  acceptance harness are present.
- `main` carries evidence-terminal gallery images but cannot regenerate or
  verify them without the unmerged implementation branch. That correction is
  preserved separately on `docs/evidence-terminal-main-claims`.
...
- ADR 0063 is accepted and implemented on `agent/physical-evidence-terminal`.
- This branch contains `boot/src/evidence_log.rs`,
...
- QEMU acceptance passes on this branch with `QEMU_OUTCOME success` and
  `EVIDENCE_TERMINAL_TEST_OK`.
- Physical evidence-terminal validation remains pending until the owner boots
  the generated image on the disposable O2 Micro `1217:8620` target and
  supplies fresh photo/video evidence.
...
- `main` and `agent/physical-evidence-terminal` remain separate lines of work.
- `main` previously documented the evidence terminal as if its own source tree
  could produce it. The branch `docs/evidence-terminal-main-claims` preserves a
  docs-only correction for that claim boundary.
- This PythTIG branch starts from the implementation line instead of silently
  resolving or discarding main's documentation conflict.
- No merge to `main` has been attempted from this branch.
- The owner still needs to decide whether to review/merge
  `docs/evidence-terminal-main-claims`, `agent/physical-evidence-terminal`,
  and this PythTIG proposal as separate steps.

## Baseline Test Results

Command run from isolated worktree after docs-only import on 2026-08-04:
...
The harness also built and verified the user shell ELF, built the ISO, booted
QEMU with `--no-virtio-blk --sdhci --emmc`, and captured
...
- Whether and when to merge the main claim-correction branch.
- Whether and when to merge `agent/physical-evidence-terminal`.
