...
The active implementation is in Phase 7 `persistent-object-storage`. The `typed-object-format` slice is complete and ADR 0022 records the on-disk format. The next allowed slice is `object-relationships`. Do not begin revision history, workspace objects, object browser work, networking, AI, ring-3, SMP, or hostile-code isolation before their roadmap gates.
