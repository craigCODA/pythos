...
The active implementation is in Phase 7 `persistent-object-storage`. The `workspace-objects` slice is complete; ADR 0022 records the on-disk format and ADR 0023 records the workspace-session object kind. The next allowed slice is `object-browser`. Do not begin save/restore across reboot, networking, AI, ring-3, SMP, or hostile-code isolation before their roadmap gates.
