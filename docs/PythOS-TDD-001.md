...
performs one fixed volatile status read. QEMU `e1000`/`e1000e` passed the
`0x08` read; physical Lenovo observation of the target `0x00F4` status location
is pending. This does not claim physical NIC/Wi-Fi operation or a generalized
MMIO abstraction.
...
* ADR 0066 supersedes the desktop/window/widget/application authority portions of Phase 5 and related Phase 7 descriptions. Existing slice names, test-contract names, durable object-kind codes, and serial marker strings remain compatibility labels and must not be renamed without an explicit migration ADR and reboot/replay proof.
...
* `software-renderer` validates bounded framebuffer-target drawing primitives by filling clipped rectangles into a fixed pixel buffer, emits `PYTHOS:CORE:RENDER:RECT`, and completes with `PYTHOS:CORE:SOFTWARE_RENDERER_READY`. It does not implement fonts, compositing, surfaces, or conventional widgets.
* `font-system` implements ADR 0019's boot-font ABI extension by passing `FONT.PSF` through explicit boot-info fields, reserving and mapping the font range, parsing PSF1 metadata, validating a bounded glyph table, emitting `PYTHOS:CORE:FONT:PSF_LOADED`, and completing with `PYTHOS:CORE:FONT_SYSTEM_READY`. It does not implement text layout, compositing, or conventional shell controls.
* `compositor` implements ADR 0018's typed object and presentation-binding split for the first projection surfaces, composes bounded surface pixels into a bounded framebuffer target, proves clipping at target edges, emits `PYTHOS:CORE:COMPOSITOR:SURFACE` and `PYTHOS:CORE:COMPOSITOR:CLIP`, and completes with `PYTHOS:CORE:COMPOSITOR_READY`. It does not implement pointer movement, focus, conventional widgets, or shell applications.
* `window-interaction` is now a compatibility slice name. It builds on the typed-object compositor surface with bounded cursor state, focus selection, and moving a projected surface by mutating only presentation binding, emits `PYTHOS:CORE:POINTER_CURSOR_READY`, `PYTHOS:CORE:WINDOW_FOCUS_READY`, and `PYTHOS:CORE:MOVABLE_WINDOWS_READY`. It does not authorize conventional widgets or shell applications.
* `widgets` is now a compatibility slice name. It proves fixed button activation and bounded text-field input editing as typed action controls over typed objects, emits `PYTHOS:CORE:WIDGET:BUTTON` and `PYTHOS:CORE:WIDGET:TEXT_FIELD`, and completes with `PYTHOS:CORE:WIDGETS_READY`. It does not authorize a conventional widget toolkit or first-party shell applications.
* `phase-5-complete` uses compatibility names for the fixed launcher, service monitor, Python console, and settings panel projections. It renders diagnostic, console, policy-inspection, and task/tool-initiation projections through the compositor path, emits the four `PYTHOS:CORE:APP:*` markers, and completes Phase 5 with `PYTHOS:CORE:PHASE_5_COMPLETE`. It does not implement Phase 6 audio, cinematic boot, or a desktop/application user model.
...
* `block-device-driver` attaches an explicit boot ESP plus a non-boot QEMU legacy `virtio-blk` raw storage image, scans the primary PCI bus for vendor `0x1AF4` device `0x1001`, validates the legacy I/O BAR, enables I/O and bus-master command bits, reads bounded capacity and queue metadata, emits `PYTHOS:CORE:BLOCK:DEVICE_SELECTED` plus `PYTHOS:CORE:BLOCK:DEVICE_SELECTED_VIRTIO`, and completes with `PYTHOS:CORE:BLOCK_DEVICE_READY`. It does not implement storage-service mediation, data I/O, journaling, object records, or crash recovery.
* ADR 0054 extends the block backend boundary with a polling AHCI fallback, without changing the original virtio default. PythCore discovers AHCI by PCI class `0x01` / subclass `0x06` / prog-if `0x01`, maps BAR5/ABAR into a fixed uncacheable kernel virtual window before the VM switch, starts one plain-SATA port with one command slot, issues single-sector ATA `READ DMA EXT` / `WRITE DMA EXT` requests, emits `PYTHOS:CORE:BLOCK:AHCI_CONTROLLER_FOUND` and `PYTHOS:CORE:BLOCK:DEVICE_SELECTED_AHCI`, and is accepted by `python scripts\test-ahci-block-device.py` with virtio disabled across two boots. It does not implement NVMe, interrupt-driven storage, MSI/MSI-X, Local APIC/IOAPIC, multi-bus PCI enumeration, ATA IDENTIFY capacity discovery, partitions, filesystems, hotplug, IOMMU/DMA isolation, networking, package management, SMP, or AI.
...
* `workspace-objects` records ADR 0023's first concrete persistent object kind, `WorkspaceSession`, and stores the Phase 5 compatibility projection layout as bounded ADR 0022 fields. It emits `PYTHOS:CORE:WORKSPACE:SESSION_OBJECT`, `PYTHOS:CORE:WORKSPACE:WINDOW_LAYOUT`, and completes with `PYTHOS:CORE:WORKSPACE_OBJECTS_READY`. It does not implement object browser work, reboot persistence, or sector persistence.
* `object-browser` records ADR 0024's minimal inspection boundary and exposes a fixed Phase 5 typed projection over the current object-store substrate. It lists typed objects, inspects typed relationships, inspects revision counts, emits `PYTHOS:CORE:OBJECT_BROWSER:LIST`, `PYTHOS:CORE:OBJECT_BROWSER:DETAIL`, and completes with `PYTHOS:CORE:OBJECT_BROWSER_READY`. It does not implement reboot persistence or sector persistence.
...
* `crash-containment` records ADR 0035's Phase 8 crash containment proof. PythCore runs a fixed CPL3 illegal-instruction probe, diagnoses it through the exception path as a user fault, terminates only the faulting service process, preserves a peer service process, emits `PYTHOS:CORE:CRASH:USER_FAULT`, `PYTHOS:CORE:CRASH:SERVICE_TERMINATED`, `PYTHOS:CORE:CRASH:PEER_ALIVE`, and completes with `PYTHOS:CORE:CRASH_CONTAINMENT_READY`. It does not implement hostile-code capability enforcement at the syscall boundary.
* `capability-enforcement-at-boundary` records ADR 0036's final Phase 8 boundary proof. PythCore contains a fixed CPL3 bad-pointer page fault, validates legitimate syscall-gated capability use before IPC mutation, denies a copied handle value from the wrong service identity, denies hardware-resource repurposing, emits `PYTHOS:CORE:BOUNDARY:BAD_POINTER_CONTAINED`, `PYTHOS:CORE:BOUNDARY:CAPABILITY_ALLOWED`, `PYTHOS:CORE:BOUNDARY:FORGERY_DENIED`, `PYTHOS:CORE:BOUNDARY:HARDWARE_DENIED`, and completes with `PYTHOS:CORE:CAPABILITY_BOUNDARY_READY`. It does not implement copy-in/copy-out, dynamic userspace, networking, package management, SMP, or hardware expansion.
...
* `block-allocator` records ADR 0044's Phase 10 allocator format. PythCore proves bitmap allocation, journaled allocator metadata, torn metadata rollback, and completes with `PYTHOS:CORE:BLOCK_ALLOCATOR_READY`.
* `dynamic-object-count` creates and deletes multiple typed object records over allocator-owned extents, emits `PYTHOS:CORE:DYNAMIC_OBJECT:CREATED`, `PYTHOS:CORE:DYNAMIC_OBJECT:DELETED`, and completes with `PYTHOS:CORE:DYNAMIC_OBJECT_COUNT_READY`.
* `fragmentation-and-compaction-policy` records ADR 0045. PythCore proves freed dynamic-object extents are reused, explicitly defers live-object compaction, emits `PYTHOS:CORE:FRAGMENTATION:POLICY_RECORDED`, `PYTHOS:CORE:FRAGMENTATION:FREED_BLOCK_REUSED`, and completes with `PYTHOS:CORE:FRAGMENTATION_COMPACTION_POLICY_READY`.
* `storage-quota-per-service` extends quota accounting to storage blocks, proves an over-quota write is denied without mutating recorded usage, emits `PYTHOS:CORE:STORAGE_QUOTA:GRANTED`, `PYTHOS:CORE:STORAGE_QUOTA:DENIED`, and completes with `PYTHOS:CORE:STORAGE_QUOTA_PER_SERVICE_READY`.
* `concurrent-write-safety` serializes capability-gated storage writers with service-owned write tokens, rejects stale tokens from releasing another writer's gate, emits `PYTHOS:CORE:CONCURRENT_WRITE:SERIALIZED`, `PYTHOS:CORE:CONCURRENT_WRITE:CORRUPTION_DENIED`, and completes with `PYTHOS:CORE:CONCURRENT_WRITE_SAFETY_READY`.
* `storage-adversarial-suite` proves repeated create/delete cycles, out-of-quota denial, dynamic torn-write rollback, and an external killed-mid-commit recovery path in `scripts/test-persistent-storage.py`. It emits `PYTHOS:CORE:STORAGE_ADVERSARIAL:CREATE_DELETE_CYCLE`, `PYTHOS:CORE:STORAGE_ADVERSARIAL:OUT_OF_QUOTA_DENIED`, `PYTHOS:CORE:STORAGE_ADVERSARIAL:DYNAMIC_TORN_WRITE_RECOVERED`, completes with `PYTHOS:CORE:STORAGE_ADVERSARIAL_SUITE_READY`, and completes Phase 10 with `PYTHOS:CORE:PHASE_10_COMPLETE`.
...
Phase 8 requires `PYTHOS:CORE:USER_MODE:ENTER` after `PYTHOS:CORE:PHASE_7_COMPLETE`, `PYTHOS:CORE:USER_MODE:RETURN` after the enter marker, `PYTHOS:CORE:RING3_EXECUTION_READY` after the return marker, `PYTHOS:CORE:ADDRESS_SPACE:CREATED` after `PYTHOS:CORE:RING3_EXECUTION_READY`, `PYTHOS:CORE:ADDRESS_SPACE:ISOLATED` after the created marker, `PYTHOS:CORE:ADDRESS_SPACE:SWITCHED` after the isolated marker, a second `PYTHOS:CORE:USER_MODE:ENTER`/`PYTHOS:CORE:USER_MODE:RETURN` pair after the address-space switch, `PYTHOS:CORE:ADDRESS_SPACE:RESTORED` after the second return marker, `PYTHOS:CORE:SEPARATE_ADDRESS_SPACES_READY` after the restored marker, `PYTHOS:CORE:SYSCALL:MSRS_READY` after `PYTHOS:CORE:SEPARATE_ADDRESS_SPACES_READY`, `PYTHOS:CORE:SYSCALL:ENTER` after MSR setup, `PYTHOS:CORE:SYSCALL:CAPABILITY_CHECK` after syscall entry, `PYTHOS:CORE:SYSCALL:SYSTEM_LOG` after the capability check, `PYTHOS:CORE:SYSCALL:RETURN` after the system-log marker, `PYTHOS:CORE:SYSCALL_ENTRY_READY` after syscall return, `PYTHOS:CORE:USER_STACK:ALLOCATED` after `PYTHOS:CORE:SYSCALL_ENTRY_READY`, `PYTHOS:CORE:USER_STACK:GUARD_PAGE` after the allocation marker, `PYTHOS:CORE:USER_STACKS_READY` after the third Phase 8 user-mode return marker, `PYTHOS:CORE:RUNTIME:LOCAL_INSTANCE` after `PYTHOS:CORE:USER_STACKS_READY`, `PYTHOS:CORE:RUNTIME:ADDRESS_SPACE` after the local-instance marker, `PYTHOS:CORE:RUNTIME:STATE_ISOLATED` after the runtime address-space marker, `PYTHOS:CORE:SERVICE_LOCAL_RUNTIMES_READY` after the state-isolated marker, `PYTHOS:CORE:SHM:RING3_READ` after `PYTHOS:CORE:SERVICE_LOCAL_RUNTIMES_READY`, `PYTHOS:CORE:SHM:CROSS_SPACE_WRITE_DENIED` after the ring-3 read marker, `PYTHOS:CORE:GUARDED_SHARED_MEMORY_READY` after the denied write marker, `PYTHOS:CORE:PROCESS:TERMINATED` after `PYTHOS:CORE:GUARDED_SHARED_MEMORY_READY`, `PYTHOS:CORE:PROCESS:UNSCHEDULABLE` after the terminated marker, `PYTHOS:CORE:PROCESS:ADDRESS_SPACE_RECLAIMED` after the unschedulable marker, `PYTHOS:CORE:PROCESS_TERMINATION_READY` after the address-space-reclaimed marker, `PYTHOS:CORE:QUOTA:MEMORY_GRANTED` after `PYTHOS:CORE:PROCESS_TERMINATION_READY`, `PYTHOS:CORE:QUOTA:MEMORY_DENIED` after the memory-granted marker, `PYTHOS:CORE:MEMORY_QUOTAS_READY` after the memory-denied marker, `PYTHOS:CORE:QUOTA:CPU_TICK` after `PYTHOS:CORE:MEMORY_QUOTAS_READY`, `PYTHOS:CORE:QUOTA:CPU_THROTTLED` after the CPU-tick marker, `PYTHOS:CORE:CPU_QUOTAS_READY` after the CPU-throttled marker, `PYTHOS:CORE:CRASH:USER_FAULT` after `PYTHOS:CORE:CPU_QUOTAS_READY`, `PYTHOS:CORE:CRASH:SERVICE_TERMINATED` after the user-fault marker, `PYTHOS:CORE:CRASH:PEER_ALIVE` after the terminated marker, `PYTHOS:CORE:CRASH_CONTAINMENT_READY` after the peer-alive marker, `PYTHOS:CORE:BOUNDARY:BAD_POINTER_CONTAINED` after crash containment, `PYTHOS:CORE:BOUNDARY:CAPABILITY_ALLOWED` after the bad-pointer marker, `PYTHOS:CORE:BOUNDARY:FORGERY_DENIED` after the allowed marker, `PYTHOS:CORE:BOUNDARY:HARDWARE_DENIED` after the forgery-denied marker, `PYTHOS:CORE:CAPABILITY_BOUNDARY_READY` after the hardware-denied marker, and `PYTHOS:CORE:CAPABILITY_BOUNDARY_READY` before the Phase 9 `dynamic-elf-loading` markers.

Phase 9 `dynamic-elf-loading` requires `PYTHOS:CORE:USER_ELF:REJECTED:BUFFER_RANGE` after `PYTHOS:CORE:CAPABILITY_BOUNDARY_READY`, `PYTHOS:CORE:USER_ELF:REJECTED:WX_SEGMENT` after the buffer-range rejection marker, `PYTHOS:CORE:USER_ELF:REJECTED:KERNEL_RANGE` after the W+X rejection marker, `PYTHOS:CORE:USER_ELF:LOADED` after the kernel-range rejection marker, `PYTHOS:CORE:USER_ELF:SEGMENTS_MAPPED` after the loaded marker, `PYTHOS:CORE:DYNAMIC_ELF_LOADING_READY` after the segments-mapped marker, and `PYTHOS:CORE:DYNAMIC_ELF_LOADING_READY` before the Phase 9 `general-syscall-abi` markers.
...
Phase 10 `block-allocator` requires `PYTHOS:CORE:ALLOCATOR:BITMAP_READY` after `PYTHOS:CORE:PHASE_9_COMPLETE`, `PYTHOS:CORE:ALLOCATOR:METADATA_JOURNALED` after the bitmap marker, `PYTHOS:CORE:ALLOCATOR:TORN_METADATA_ROLLED_BACK` after the journaled metadata marker, and `PYTHOS:CORE:BLOCK_ALLOCATOR_READY` after the torn-metadata rollback marker.

Phase 10 `dynamic-object-count` requires `PYTHOS:CORE:DYNAMIC_OBJECT:CREATED` after `PYTHOS:CORE:BLOCK_ALLOCATOR_READY`, `PYTHOS:CORE:DYNAMIC_OBJECT:DELETED` after the created marker, and `PYTHOS:CORE:DYNAMIC_OBJECT_COUNT_READY` after the deleted marker.

Phase 10 `fragmentation-and-compaction-policy` requires `PYTHOS:CORE:FRAGMENTATION:POLICY_RECORDED` after `PYTHOS:CORE:DYNAMIC_OBJECT_COUNT_READY`, `PYTHOS:CORE:FRAGMENTATION:FREED_BLOCK_REUSED` after the policy marker, and `PYTHOS:CORE:FRAGMENTATION_COMPACTION_POLICY_READY` after the reuse marker.

Phase 10 `storage-quota-per-service` requires `PYTHOS:CORE:STORAGE_QUOTA:GRANTED` after `PYTHOS:CORE:FRAGMENTATION_COMPACTION_POLICY_READY`, `PYTHOS:CORE:STORAGE_QUOTA:DENIED` after the granted marker, and `PYTHOS:CORE:STORAGE_QUOTA_PER_SERVICE_READY` after the denied marker.
...
ADR 0088 records target-specific physical acceptance on the Lenovo `81VS`
using AMD xHCI `1022:7914` with the Dell/PixArt `413c:301a` boot mouse. Generic
USB/HID support, other devices or controllers, hubs, IRQ-driven input,
hot-unplug recovery, and ADR 0089 Viewing behavior remain outside that evidence.
