...
The active implementation is in Phase 7 `persistent-object-storage`. The `object-browser` slice is complete; ADR 0022 records the on-disk format, ADR 0023 records the workspace-session object kind, and ADR 0024 records the object-browser inspection boundary. The next allowed slice is `save-and-restore-across-reboot`. Do not begin Phase 8, networking, AI, ring-3, SMP, or hostile-code isolation before their roadmap gates.
