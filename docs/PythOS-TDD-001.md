...
* `system-api-surface` exposes ADR 0016's first `system.*` function, `system.log(message)`, validates a `LOG` capability for the runtime service identity, rejects empty or oversized messages, emits `PYTHOS:CORE:SYSTEM:LOG`, and completes with `PYTHOS:CORE:SYSTEM_API_READY`. It does not implement service readiness or the general value-validation layer.
