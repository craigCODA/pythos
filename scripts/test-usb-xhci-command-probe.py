#!/usr/bin/env python
"""Acceptance test for the first write/DMA USB/xHCI command-ring diagnostic."""
...
TARGET = ROOT / "target"
SERIAL_LOG = TARGET / "usb-xhci-command-probe-com1.log"
USB_BOOT_SIM_IMAGE = TARGET / "pythos-xhci-command-boot-sim.img"
...
def assert_forbidden_markers_absent(serial: str) -> None:
    for marker in FORBIDDEN_MARKERS:
        if marker in serial:
            raise AssertionError(f"command probe boot unexpectedly reached marker: {marker}")


def assert_command_driver_result(serial: str) -> None:
...
def main() -> int:
    ...
    assert_command_driver_result(serial)
    print("USB_XHCI_COMMAND_PROBE_TEST_OK")
    return 0
